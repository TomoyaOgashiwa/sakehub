...
import { quoteLiteral, quoteNullableLiteral, quoteNullableNumber } from './sql.ts';
...
async function main(): Promise<void> {
  ...
  const lines: string[] = [];
  lines.push(`-- =============================================================================`);
  lines.push(`-- AUTO-GENERATED by packages/cocktail-seed (src/build-seed.ts).`);
  lines.push(`-- Do not edit by hand — regenerate with: pnpm seed:cocktails:build`);
  lines.push(`-- Generated at: ${new Date().toISOString()}`);
  lines.push(`-- Cocktail count: ${cocktails.length}`);
  lines.push(`-- =============================================================================`);
  lines.push('');
  ...
  for (const c of cocktails) {
    ...
    lines.push(`-- ${c.slug}`);
    lines.push(`INSERT INTO cocktails (id, slug, name, name_en, description, base_spirit, abv, origin_country)`);
    lines.push(`VALUES (`);
    ...
    lines.push(`INSERT INTO cocktail_recipes (id, user_id, cocktail_id, name, memo, status, is_official)`);
    lines.push(`VALUES (`);
    ...
    lines.push(`DELETE FROM cocktail_recipe_ingredients WHERE recipe_id = ${quoteLiteral(recipeId)};`);
    lines.push(`DELETE FROM cocktail_recipe_steps WHERE recipe_id = ${quoteLiteral(recipeId)};`);
    lines.push('');

    if (recipe.ingredients.length > 0) {
      lines.push(`INSERT INTO cocktail_recipe_ingredients (recipe_id, name, amount, unit, sort_order) VALUES`);
