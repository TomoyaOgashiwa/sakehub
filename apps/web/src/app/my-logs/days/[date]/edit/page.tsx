...
import { fetchAllMyDrinkLogsInRange } from '@/application/drink-logs-api.server';
...
export default async function EditDrinkLogsForDayPage({ params }: PageProps) {
  ...
  const logs = await fetchAllMyDrinkLogsInRange(accessToken, rangeFrom, rangeTo);
  if (logs.length === 0) {
    notFound();
  }
      ...
      {overLimit ? (
        <div className="flex flex-col gap-4">
          <p className="text-muted-foreground text-sm">
            この日の記録は {logs.length} 件あり、一括編集の上限（
            {DRINK_LOG_MAX_ITEMS_PER_BATCH}{' '}
            件）を超えています。1件ずつ編集するか、不要な記録を削除してください。
          </p>
