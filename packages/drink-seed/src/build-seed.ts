...
async function main(): Promise<void> {
  ...
  const lines: string[] = [];
  lines.push(`-- =============================================================================`);
  lines.push(`-- AUTO-GENERATED by packages/drink-seed (src/build-seed.ts).`);
  lines.push(`-- Do not edit by hand — regenerate with: pnpm seed:drinks:build`);
  lines.push(`-- Generated at: ${new Date().toISOString()}`);
  lines.push(`-- Drink count: ${drinks.length}`);
  lines.push(`--`);
  ...
  for (const d of drinks) {
    ...
    lines.push(`)`);
    // slug is UNIQUE NOT NULL (create_drinks.sql), so re-running the pipeline
    // against a non-fresh DB (e.g. after Phase 3 approvals) upserts in place
